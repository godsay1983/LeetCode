...
vector<int> Solution::flatten(vector<NestedInteger>& nestedList)
{
	// Write your code here
	vector<int> intList;
	doFlat(intList, nestedList);
	return intList;
}

void Solution::doFlat(vector<int>& intList, const vector<NestedInteger>& nestedList)
{
	for (auto nestedInteger : nestedList)
	{
		if (nestedInteger.isInteger())
		{
			intList.push_back(nestedInteger.getInteger());
		}
		else
		{
			doFlat(intList, nestedInteger.getList());
		}
	}
}
